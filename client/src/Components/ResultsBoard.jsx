        ...
        <tbody>
            {data.services.map((service) => (
                <tr onClick={() => handleClick(service.serviceUid)} key={service.serviceUid} className="odd:bg-white even:bg-gray-200 hover:bg-gray-300 cursor-pointer">
                    ...
                    <td>{service.serviceType === "bus" || service.serviceType === "ship" ? "See station signage" : service.locationDetail.displayAs === "CANCELLED_CALL" ? "Cancelled" : (service.locationDetail.realtimeDeparture).slice(0,2) + ":" + (service.locationDetail.realtimeDeparture).slice(2,4)}</td>
